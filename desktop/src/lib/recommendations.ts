...
const DESCRIPTION_WEIGHT = 0.78;
const GENRE_WEIGHT = 0.1;
const DESCRIPTION_SCALE = 2.4;
const MINIMUM_RECOMMENDATION_SCORE = 0.1;
...
export function tasteScore(
  ...
  const seedScores = seeds.map((seed) => {
    const rawDescriptionSimilarity = descriptionSimilarity(candidate, seed, descriptionContext);
    const descriptionAffinity = Math.min(rawDescriptionSimilarity * DESCRIPTION_SCALE, 1);
    const genreSimilarity = jaccard(new Set(seed.genreIds), new Set(candidate.genreIds));
    const mediaAffinity = seed.mediaType === candidate.mediaType ? 0.02 : 0.005;
    const seedYear = releaseYear(seed);
    const candidateYear = releaseYear(candidate);
    const yearAffinity =
      seedYear && candidateYear
        ? Math.max(0, 1 - Math.abs(seedYear - candidateYear) / 35) * 0.015
        : 0;

    return (
      descriptionAffinity * DESCRIPTION_WEIGHT
  ...
  const quality = (candidate.voteAverage / 10) * ratingConfidence * 0.04;
  const popularity = Math.min(Math.log10(candidate.popularity + 1) / 4, 1) * 0.005;
  const likedBoost = signals.get(candidate.key) === "liked" ? 0.1 : 0;
  return closestSeed + quality + popularity + likedBoost;
}
...
export function rankTasteMatches(
  ...
  const descriptionContext = buildDescriptionContext([...catalog, ...seeds]);
  return [...catalog]
    .map((item) => ({ item, score: tasteScore(item, seeds, signals, descriptionContext) }))
